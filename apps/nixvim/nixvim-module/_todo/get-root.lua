-- Root directory detection module
...
-- Default root spec: LSP, then .git/lua, then cwd
...
-- Setup autocmd for cache clearing
function M.setup()
  ...
  for i, r in ipairs(roots) do
    for _, path in ipairs(r.paths) do
      lines[#lines + 1] = string.format(
        "%s [%s] %s",
        i == 1 and "→" or " ",
        type(r.spec) == "table" and table.concat(r.spec, ", ") or r.spec,
        path
      )
    end
  end
  vim.notify(table.concat(lines, "\n"), vim.log.levels.INFO)
end, { desc = "Show root directory info" })
